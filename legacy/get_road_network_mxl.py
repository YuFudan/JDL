...
def plot_road_network(roads, nodes, buildings=None, path="figure_mxl/test.html"):
...
def plot_G(G, path="figure_mxl/test.html"):
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']
    G_folium = folium.Map(
        location=[lat_cen, lon_cen],
        control_scale=True,
...
def construct_graph(roads, nodes):
    ...
    if False:
        ...
        plot_G(G, "figure_mxl/G.html")

    return G


if __name__ == "__main__":
    # orig_data = json.load(open("orig_data_mxl/road_geojson.json"))["features"]
    # roads, nodes = read_road_network(orig_data)
    # pickle.dump((roads, nodes), open("data_mxl/roads_nodes.pkl", "wb"))

    roads, nodes = pickle.load(open("data_mxl/roads_nodes.pkl", "rb"))

    buildings = pickle.load(open("data_mxl/buildings.pkl", "rb"))

    # plot_road_network(roads, nodes, buildings, "figure_mxl/road_network.html")

    roads, nodes = match_building_to_road(roads, nodes, buildings)

    station_gps = gcj2wgs(116.5228, 39.8101)
    ...
    pickle.dump(G, open("data_mxl/G.pkl", "wb"))
