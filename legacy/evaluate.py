...
def add_stall_to_map(G, buildings, cid2stall_info):
    ...
    # TODO:
    communities = pickle.load(open("/home/yufudan/jd_demo1/mxl/data/poi_community.pkl", "rb"))[1]
...
def data_prepare():
    cache = f"data/cache_data_prepare_{len(TRAIN_DATES)}.pkl"
    if os.path.exists(cache):
        ...
        waves1 = pickle.load(open("data/wave_data.pkl", "rb"))
        waves2 = pickle.load(open("data/wave_data_nofilter.pkl", "rb"))
        train_data = get_train_test_data(waves1)[0]
        train_data2, test_data2 = get_train_test_data(waves2)[0]
    ...
    # 准备测试数据
    test_data = get_train_test_data(pickle.load(open("data/wave_data_nofilter.pkl", "rb")))[1]
...
def cal_gt_metric(wave, bellman_ford, verbose=False):
    """计算真值指标"""
    def cal_work_time(wave):
        ...
        if verbose:
            print("真值时长修正1:", t_remove1)
            print("真值时长修正2:", t_remove2)
...
def cal_sim_metric(sim_actions, verbose=False):
    """
    ...
    for x in sim_actions:
        ...
        for a in actions:
            ...
            if a.get("wait_for_pick", False):
                ...
                if verbose:
                    atp2wt[a["type"]] += a["end_time"] - a["start_time"]
        ...
        if verbose:
            atp2wt = {k: round(100 * v / work_time, 2) for k, v in atp2wt.items()}
            print("模拟工作时长构成:")
            pprint(sorted(list(atp2wt.items()), key=lambda x: -x[1]))
            print("模拟指标:")
            pprint({
                "wt": round(work_time),
...
def merge_one_day_metrics(ms):
    ...
    for (cid, date), ms in cid_date2ms.items():
        ...
        oid2idx = ms[0]["seq"]
        for m in ms[1:]:
            oid2idx.update(m["seq"])  # 不用给后面的加offset
        m_merge["seq"] = oid2idx
        if "dcbnum" in ms[0]:
...
def cal_df_metric(gt_metrics, sim_metrics, ignore_cids=set(), merge_wave=True):        
    def dseq_exp(N):
        """
    ...
    #     l_gt = sum(m["tl"] for m in ms_gt if m["tl"] is not None)
    #     if l_gt > 0:
    #         ms_sim = sim[cid]
    #         l_sim = sum(m["tl"] for m in ms_sim if m["tl"] is not None)
    #         p = l_gt / l_sim
    #         for m in ms_sim:
    #             if m["tl"] is not None:
    #                 m["tl"] *= p
    ...
    for (cid, date, wid), msim in sim_metrics.items():
        ...
        lines.append([
            str(date), str(wid), str(cid), cid2name.get(cid, "无名"), 
            dwt, dwt_rel, 
            deff, deff_rel, 
...
class Evaluater:
    ...
    def baseline_avg(self):
        ...
        self.df_metric_avg = cal_df_metric(self.test_metrics, metrics, ignore_cids=self.ignore_cids, merge_wave=self.merge_wave)
        return metrics
    ...
    def baseline_xgb(self):
        ...
        self.df_metric_xgb = cal_df_metric(self.test_metrics, metrics, ignore_cids=self.ignore_cids, merge_wave=self.merge_wave)
        return metrics
...
def plot_train_test_curve(train_gt, train_sim, test_gt, test_sim, merge_wave=True):
    ...
    for cid in tqdm(test_gt):
        ...
        plt.savefig(f"figure/metric_{cid2name[cid]}.png", dpi=200)


def data_prepare_float():
    train_data_base, cid2stall_info, bellman_ford, _ = pickle.load(open("data/cache_data_prepare_21.pkl", "rb"))
    ...
    for n in tqdm(range(1, 30)):
        ...
        pickle.dump(
            ...
            open(f"data/eval_datas_float_{n}.pkl", "wb"))
...
def plot_box_float(step2ms):
    ...
    ms = [m if len(m) == 30 else m + [[-1]*7 for _ in range(30 - len(m))] for m in ms]  # 补零以对齐window数
    ...
    for m, name in zip(ms, return_keys):
        ...
        plt.title("误差_" + name, fontproperties=font, fontsize=15)
        plt.gca().xaxis.set_major_locator(MultipleLocator(1))

        # 画箱型图
        plt.boxplot(step2wms.values(), labels=steps, positions=range(1, len(steps) + 1))
        plt.savefig(f"figure/step_{name}.png")
...
if __name__ == "__main__":
    ...
    # step2ms = pickle.load(open("data/step2ms.pkl", "rb"))
    ...
    #         sim_actions = pickle.load(open(f"data/sim_actions.pkl", "rb"))
    #         sim_actions = {k: [x for x in v if x["date"] in TEST_DATES] for k, v in sim_actions.items()}
    #     else:
    #         train_data, test_data, cid2stall_info, bellman_ford = pickle.load(open(f"data/eval_datas_float_{n}.pkl", "rb"))
    #         sim_actions = pickle.load(open(f"data/bayes_actions_float_{n}.pkl", "rb"))
    ...
    # pickle.dump(step2ms, open("data/step2ms.pkl", "wb"))
    # exit()
    ...
    # sim_actions
    # sim_actions = pickle.load(open(f"data/sim_actions_lkh.pkl", "rb"))
    sim_actions = pickle.load(open(f"data/sim_actions_stat.pkl", "rb"))
    # sim_actions = pickle.load(open(f"data/sim_actions_nn.pkl", "rb"))
    sim_actions = {k: [x for x in v if x["date"] in TEST_DATES] for k, v in sim_actions.items()}
    assert len(train_data) == len(test_data) == len(sim_actions)
    ...
    # # 画train-test图
    # train_sim_actions = pickle.load(open("data/train_sim_actions.pkl", "rb"))
    ...
    if mode == "date_wise":  # 误差-预测时间步 趋势图
        ...
        for data, name in zip(ms, return_keys):
            ...
            # for x in data:
            #     plt.boxplot(x[1:])
            plt.savefig(f"figure/metric_date_{name}.png")
    elif mode == "courier_wise":     # 小哥误差CDF图
        cid2ms = r
        font = FontProperties(fname=r"../msyh.ttc")
        e, s, c, b = zip(*list(cid2ms.values()))
        for data, name in zip([e, s, c, b], ["工作效率", "送单次序", "C揽及时率", "B揽及时率"]):
            if name == "送单次序" or name == "工作效率":  # TODO:
                data = [x for x in data if x < 40]
            x, y = zip(*cum(data))
        ...
        plt.savefig(f"figure/courier_CDF.png")
