...
def get_waves(orders, traj=None):
    ...
    if len(start_times) < 20:  # TODO: 目前依赖于派件收货时间来判断波的起止时间, 若小哥全是揽收则失败
        return [], None, None
    ...
    for i, (t1, n1) in enumerate(finish_points):  
        ...
        try:
            find_cum_at_t(receive_points, t2)
        except:
            print(len(orders))
            print(set([o["type"] for o in orders]))
            exit()
    ...
    for i, (s, e) in enumerate(waves_refine):
        s_new = s
        if i > 0:  # 第一波开始不动
            s_min = max(s - 900, waves_refine[i-1][1] if i > 0 else 0)  # 避免找回太多
            for x, y, t in traj_upsampled[::-1]:
                if s_min < t < s:
        ...
        if i < len(waves_refine) - 1:  # 最后一波结束不动
            e_max = min(e + 900, waves_refine[i+1][0] if i < len(waves_long) - 1 else 86400)
            for x, y, t in traj_upsampled:
                if e < t < e_max:
                    if (x - X_STA) ** 2 + (y - Y_STA) ** 2 > 2 * NEAR_STA_DIS2:
                        e_new = t
        ...
        for (st, et), (so, eo) in zip(waves_traj, waves_order):
            ...
            if len(tj) < 5:  # 本应被bad_traj_tm过滤掉, 若do_filter=False则需特殊处理
                ...
                if do_filter and noise_portion > FILTER_WAVE_NOISE:
                    ...
                    continue
                good_traj += 1
                tj_new, stays = get_stay_points(tj_denoise, do_plot, f"figure/stay_{cid}_{date}_{wave_idx}.html")
