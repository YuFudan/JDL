...
def read_road_network(path):
    ...
    """
    lines = [x[0] for x in pd.read_csv(path, usecols=["WKT"]).values]
    pss = []
    for line in lines:
        assert line[:15] == "MULTILINESTRING", line[:15]
        for l in  line[18:-2].split("),("):
            ps = [x.split(" ") for x in l.split(",")]
            ps = [gcj2wgs(float(lon), float(lat)) for lon, lat in ps]
            pss.append(ps)
    ...
    for rid, s2p in tqdm(rid2s2p.items()):
        ...
        last_p = None
        for lon, lat in ps:
            p = (round(lon, 6), round(lat, 6))  # 限定小数位数, 后续作为key
            if p != last_p:
                ps_new.append(p)
                last_p = p
        r["gps"] = ps_new
...
def special_process(roads, nodes, station_gps):
    """
    如果路网连通性有问题, 可能是原始数据缺路, 根据可视化结果手动连一些边
    并同时将快递站连到路网上
    """
    for r in roads.values():
        del r["id"]
    ...
    # 将快递站连到路网(此处先把node画出来, 画图, 再看连到哪个node上合适)
    nodes["快递站"] = {
        "id": "快递站",
        "point": station_gps
    }
    nid_to_station = 82
    roads[("快递站", nid_to_station)] = {
        "od": ("快递站", nid_to_station),
        "points": [station_gps, nodes[nid_to_station]["point"]]
    }
...
if __name__ == "__main__":
    # roads, nodes = read_road_network("orig_data/road.csv")
