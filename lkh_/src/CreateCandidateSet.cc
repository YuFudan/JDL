...
End_CreateCandidateSet:
    ...
    if (MaxTrials > 0 ||
        ...
        Na = FirstNode;
        do {
            if (Na->CandidateSet.empty() || !Na->CandidateSet[0].To) {
                if (MaxCandidates == 0)
                    eprintf("MAX_CANDIDATES = 0: Node %ld has no candidates",
                            Na->Id);
                else
                    eprintf("Node %ld has no candidates", Na->Id);
            }
        } while ((Na = Na->Suc) != FirstNode);
    ...
    if (TraceLevel >= 1) {
        CandidateReport();
        printff("Preprocessing time = %0.6f sec.\n",
                fabs(GetTime() - EntryTime));
    }
}
