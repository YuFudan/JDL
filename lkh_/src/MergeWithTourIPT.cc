...
// 做IPT，T1由Suc指定，T2由Next指定
GainType MergeWithTourIPT() {
    // int64_t Rank = 0, Improved1 = 0, Improved2 = 0;
    // int64_t SubSize1, SubSize2, MaxSubSize1, NewDimension = 0, Forward;
    // int64_t MinSubSize, BestMinSubSize = 3, MinForward = 0;
    ...
    GainType Cost1 = 0, Cost2 = 0;
    Node *N, *First;
    int64_t NewDimension = 0;
    ...
    do {
        Cost2 += N->NextCost = N->Next == N->Pred ? N->Pred->Cost : N->Next == N->Suc ? N->Cost : C(N, N->Next) - N->Pi - N->Next->Pi;
    } while ((N = N->Suc) != FirstNode);

    auto OldCost1 = Cost1;

    // 计算删除相同点后的回路
    // T1'用OldPred和OldSuc表示
    // T2'用Prev和Next表示
    // First是回路中取的一点，作为起点，Rank从它开始按T1'中的顺序编号
    N = First;
    Node *Last = nullptr;
    int64_t Rank = 0;
    do {
    ...
    // 合并回路
    bool Improved1 = false, Improved2 = false;
    do {
        int64_t MinSubSize = NewDimension / 2, BestMinSubSize = 3;
        Node *N1 = First, *N2, *MinN1 = nullptr, *MinN2;
        GainType MinGain = 0;
        bool MinForward;
        do {
            // 找一个N1的OldSuc在外
            ...
            if (N1->OldSuc == First &&
                ...
                break;
            N2 = N1->Next;
            for (bool Forward = true; Forward; Forward = !Forward, N2 = N1->Prev) {
                // 找一个N1->N2在外
                if (N2 == N1->OldSuc || N2 == N1->OldPred)
                    continue;
                int64_t SubSize2 = 0, MaxSubSize1 = 0;
                do {
                    if (++SubSize2 >= MinSubSize)
                        break;
                    int64_t SubSize1 = N2->Rank - N1->Rank;
                    if (SubSize1 < 0)
        ...
        if (MinN1) {
            ...
            if (MinGain > 0) {
                ...
                Rank = MinN1->Rank;
                for (N = MinN1; N != MinN2;) {
                    auto *NNext = MinForward ? N->Next : N->Prev;
    ...
    N->Mark = N;
    do {
        if (!N->Suc->Mark && (!N->Keep || !N->Suc->Keep))
            N->OldSuc = N->Suc;
        else if (!N->Pred->Mark && (!N->Keep || !N->Pred->Keep))
            N->OldSuc = N->Pred;
        else if (Cost1 <= Cost2) {
            if (N->OldSuc->Mark)
