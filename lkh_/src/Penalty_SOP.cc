...
GainType Penalty_SOP() {
    ...
    if (SUCC(First)->Id != First->Id + Dim_salesman) {
        do
    ...
    P = Dim_salesman - NodeSet[Dim_salesman].Loc;
    ...
    if (CurrentPenalty == 0) {
        ...
        for (i = Swaps - 1; i >= 0; i--) {
            for (j = 1; j <= 4; j++) {
                N = j == 1 ? SwapStack[i].t1 : j == 2 ? SwapStack[i].t2 : j == 3 ? SwapStack[i].t3 : SwapStack[i].t4;
                if (N->Id <= Dim_salesman) {
