...
static void alloc_memory(int64_t n);
static edge *get_edge();
static void free_edge(edge *e);

static void divide(point *p_sorted[], int64_t l, int64_t r, edge **l_ccw,
                   edge **r_cw);
...
#ifndef HAVE_SCALBNF
float scalbnf(const float x, const int64_t n)
#endif
...
void delaunay(int64_t n) {
    edge *l_cw, *r_ccw;
    point **p_sorted;
    Node *N = FirstNode;
    int64_t Duplicates = 0, i, j;
...
static void divide(point *p_sorted[], int64_t l, int64_t r,
...
static void alloc_memory(int64_t n) {
    ...
    p_array = new point[n]();
    n_free_e = 3 * n;
    e_array = e = new edge[n_free_e]();
    free_list_e = new edge *[n_free_e]();
