 ...
 * Empty entries have Cost equal to -INFINITY.
 */
...
int64_t HashSearch(HashTable* T, unsigned Hash, GainType Cost) {
    int64_t i, p;

    i = Hash % HashTableSize;
    p = Hash % 97 + 1;
    while ((T->Entry[i].Hash != Hash || T->Entry[i].Cost != Cost) && T->Entry[i].Cost != -INFINITY)
        if ((i -= p) < 0)
            i += HashTableSize;
