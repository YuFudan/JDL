...
void GEO2XYZ(double Lon, double Lat, double *X, double *Y, double *Z) {
    double lon = (int64_t)Lon + 5.0 * (Lon - (int64_t)Lon) / 3.0;
    double lat = (int64_t)Lat + 5.0 * (Lat - (int64_t)Lat) / 3.0;
...
void XYZ2GEO(double X, double Y, double Z, double *Lon, double *Lat) {
    ...
    *Lon = (int64_t)lon + 3.0 * (lon - (int64_t)lon) / 5.0;
    *Lat = (int64_t)lat + 3.0 * (lat - (int64_t)lat) / 5.0;
}
