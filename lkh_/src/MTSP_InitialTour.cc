...
GainType MTSP_InitialTour() {
    ...
    GainType Cost = 0;
    int64_t Min, d, i, Dim = Dim_salesman - Salesmen + 1;
    ...
    for (i = 1; i <= Dim; i++) {
        ...
        if (N->Special) {
            ...
            N->Cost = (C(N, Route) - N->Pi - Route->Pi);
            Route->Cost = 2 * N->Cost;
            Route->V++;
            Cost += Route->Cost;
        }
    ...
    for (i = 1; i <= Dim; i++) {
        ...
        if (N == Depot || N->Special)
            continue;
        N->Rank = (C(N, Depot) - N->Pi - Depot->Pi);
        HeapLazyInsert(N);
    }
    Heapify();
    while ((N = HeapDeleteMin())) {
        ...
        Last = Winner->Pred;
        if (MTSPObjective == MINMAX || MTSPObjective == MINMAX_SIZE)
            Winner->Cost += (C(Last, N) - Last->Pi - N->Pi) +
                            N->Cost - Last->Cost;
        else
            Winner->Cost += Min;
        Cost += Winner->Cost;
