...
int64_t SegmentSize(Node* ta, Node* tb) {
    int64_t n = !Reversed ? tb->Rank - ta->Rank : ta->Rank - tb->Rank;
...
int64_t SegmentSize(Node *ta, Node *tb) {
    Segment *Pa, *Pb;
    int64_t nLeft, nMid, nRight;

    Pa = ta->Parent;
    Pb = tb->Parent;
    if (Pa == Pb) {
        int64_t n = Reversed == Pa->Reversed ? tb->Rank - ta->Rank : ta->Rank - tb->Rank;
        return (n < 0 ? n + Dimension : n) + 1;
    }
    nLeft =
        Reversed ==
