...
static void MarkBorderPoints(int64_t CurrentSubproblem);
static void QuickSelect(Node **A, int64_t n, int64_t k);

void SolveSubproblemBorderProblems(int64_t Subproblems,
                                   GainType *GlobalBestCost) {
    Node *N;
    GainType OldGlobalBestCost;
    int64_t CurrentSubproblem;
    int64_t *SubproblemSaved;
    double EntryTime = GetTime();

    SubproblemSaved = new int64_t[Dim_salesman + 1]();
...
static void MarkBorderPoints(int64_t CurrentSubproblem) {
    double Min[3], Max[3];
    int64_t dMin, dMax, dX, dY, dZ, d, i, ActualSubproblemSize = 0, Size = 0;
    Node **A, *N;

    A = new Node *[Dim_salesman]();
    ...
    do {
        if (N->Subproblem == CurrentSubproblem ||
            ...
            N->Rank = INT_MAX;
            for (i = CoordType == THREED_COORDS ? 2 : 1; i >= 0; i--) {
                dMin = (int64_t)(fabs(Coord(N, i) - Min[i]) + 0.5);
                dMax = (int64_t)(fabs(Coord(N, i) - Max[i]) + 0.5);
                d = dMin < dMax ? dMin : dMax;
                if (d < N->Rank)
                    N->Rank = d;
            }
        } else {
            dMin = (int64_t)(fabs(Coord(N, 0) - Min[0]) + 0.5);
            dMax = (int64_t)(fabs(Coord(N, 0) - Max[0]) + 0.5);
            dX = dMin < dMax ? dMin : dMax;
            dMin = (int64_t)(fabs(Coord(N, 1) - Min[1]) + 0.5);
            dMax = (int64_t)(fabs(Coord(N, 1) - Max[1]) + 0.5);
            ...
            if (CoordType == TWOD_COORDS) {
                ...
                dMin = (int64_t)(fabs(Coord(N, 2) - Min[2]) + 0.5);
                dMax = (int64_t)(fabs(Coord(N, 2) - Max[2]) + 0.5);
...
static void QuickSelect(Node **A, int64_t n, int64_t k) {
    int64_t i, j, l = 0, r = n - 1, mid, pivot;
    Node *temp, *v;
