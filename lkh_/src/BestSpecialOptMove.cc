...
Node *BestSpecialOptMove(Node *t1, Node *t2, GainType *G0,
    ...
    GainType G1, G2, G3, G4, BestG4 = -INFINITY,
                             G5, G6, G7, G8, BestG8 = -INFINITY, NewPenalty;
    int64_t X4, X6, Case6 = 0, BestCase6 = 0, Case56, Case78, Case10,
                    BestCase10 = 0, Case;
    ...
    for (auto *Nt2 = t2->CandidateSet.data(); (t3 = Nt2->To); Nt2++) {
        ...
        for (X4 = 1; X4 <= 2; X4++) {
            ...
            if (l_opt_limit >= 3)
                for (auto *Nt4 = t4->CandidateSet.data(); (t5 = Nt4->To); Nt4++) {
                    ...
                    for (X6 = 1; X6 <= X4; X6++) {
                        ...
                        if (K < 5) {
                            if (GainCriterionUsed && G4 < t6->Cost)
                                continue;
            ...
            if (t4 != t1 && !Forbidden(t4, t1) &&
                ...
                (Swaps == 0 || SubsequentPatching)) {
                G3 = G2 - C(t4, t1);
                if ((PatchingCRestricted ? G3 > 0 && IsCandidate(t4, t1) : PatchingCExtended ? G3 > 0 || IsCandidate(t4, t1) : G3 > 0)) {
                    incl[incl[2] = 3] = 2;
                    incl[incl[4] = 1] = 4;
                    t[1] = t1;
                    t[2] = t2;
            ...
            /* Try special 4-opt */
            if (l_opt_limit >= 4 && !Forbidden(t4, t1)) {
                G3 = G2 - C(t4, t1);
                if (!Asymmetric) {
                    SelectBackward(&t6, &t5, t3, t2);
            ...
            if (l_opt_limit >= 5)
                for (Case = 1; Case <= 33; Case++) {
                    ...
                    *Gain = G8 - C(t10, t1);
                    ...
                    if (GainCriterionUsed && G8 < t10->Cost)
                        continue;
                    if (!Backtracking || Swaps > 0) {
                        if ((G8 > BestG8 || (G8 == BestG8 && !Near(t9, t10) && Near(T9, T10))) && Swaps < MaxSwaps && Excludable(t9, t10) && !InInputTour(t9, t10)) {
                            /* Ignore the move if the gain does not vary */
                            if (RestrictedSearch &&
                                ProblemType != HCP &&
                                ProblemType != HPP &&
            ...
            if (l_opt_limit >= 6)
                for (Case = 1; Case <= 8; Case++) {
                    ...
                    if (CurrentPenalty != 0 ||
                        TSPTW_Makespan ||
                        !c || G4 - c(t4, t5) - c(t6, t1) - c(t8, t9) - c(t10, t11) - c(t12, t7) > 0) {
