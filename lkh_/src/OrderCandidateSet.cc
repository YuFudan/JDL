...
static int64_t BetaValue(Node *From, Node *To);
...
void OrderCandidateSet(int64_t MaxCandidates, GainType MaxAlpha, int64_t Symmetric) {
    ...
    do {
        for (Candidate *NN, *NFrom = From->CandidateSet.data(); NFrom && (To = NFrom->To);
             NFrom++) {
    ...
    if (MaxCandidates > 0 && !DelaunayPure && CandidateSetType == DELAUNAY) {
        do {
            int64_t Count = 0;
            for (auto *NFrom = From->CandidateSet.data(); NFrom->To; NFrom++)
                Count++;
