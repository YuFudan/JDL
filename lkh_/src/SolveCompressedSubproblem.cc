...
void SolveCompressedSubproblem(int64_t CurrentSubproblem, int64_t Subproblems,
                               GainType *GlobalBestCost) {
    int64_t Level, Number, RestrictedSearchSaved = RestrictedSearch;
    ...
    for (Level = 1;
         ...
         Level++) {
        if (TraceLevel >= 1)
            printff("\nCompress subproblem %ld (Level %ld):", Number, Level);
