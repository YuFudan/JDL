...
Start:
    ...
    if (Asymmetric && FirstNode->Id <= Dim_salesman)
        FirstNode += Dim_salesman;
    ...
    while (N->Suc != FirstNode) {
        ...
        /* Case A */
        for (auto *NN = N->CandidateSet.data(); NN && (NextN = NN->To); NN++) {
            if (!NextN->chosen && Fixed(N, NextN)) {
                Alternatives++;
        ...
        if (Alternatives == 0 && MergeTourFiles > 1) {
            for (auto *NN = N->CandidateSet.data(); NN && (NextN = NN->To); NN++) {
                if (!NextN->chosen && IsCommonEdge(N, NextN)) {
                    Alternatives++;
                    NextN->Next = FirstAlternative;
                    FirstAlternative = NextN;
                }
        ...
        if (Alternatives == 0 && FirstNode->InitialSuc && Trial == 1 &&
            ...
            /* Case B */
            for (auto *NN = N->CandidateSet.data(); NN && (NextN = NN->To); NN++) {
                if (!NextN->chosen && InInitialTour(N, NextN)) {
                    Alternatives++;
                    NextN->Next = FirstAlternative;
                    FirstAlternative = NextN;
                }
        ...
        if (Alternatives == 0 && Trial > 1 &&
            ...
            /* Case C */
            for (auto *NN = N->CandidateSet.data(); NN && (NextN = NN->To); NN++) {
                if (!NextN->chosen && FixedOrCommonCandidates(NextN) < 2 &&
