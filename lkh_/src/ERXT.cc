...
static int64_t EdgeCount(Node *N) {
    int64_t Count = 0;
    Node *Next;
...
static Node *SelectNext(Node *N) {
    ...
    int64_t Alternatives = 0, Score, MaxScore = INT_MIN, i;

    for (i = 1; i <= 4; i++) {
        Next =
            i == 1 ? N->OldPred : i == 2 ? N->OldSuc : i == 3 ? N->Prev : N->Next;
