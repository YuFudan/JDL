...
GainType gpx(int64_t *solution_blue, int64_t *solution_red, int64_t *offspring) {
    int64_t i, j, *d4_vertices, n_d4_vertices, *common_edges_blue,
        *common_edges_red;
    int64_t *common_edges_p2_blue, *common_edges_p2_red, *label_list,
        *label_list_inv, n_new;
    int64_t *solution_blue_p2, *solution_red_p2, *vector_comp;
...
int64_t weight(int64_t i, int64_t j) {
    ...
    return C(a, b) - a->Pi - b->Pi;
}

// Identifying the vertices with degree 4 and common edges
int64_t d4_vertices_id(int64_t *solution_blue, int64_t *solution_red, int64_t *d4_vertices,
                       int64_t *common_edges_blue, int64_t *common_edges_red) {
    int64_t i, aux, aux2, **M_aux, n_d4_vertices;

    // create a matrix (n_cities x 4) with all edges;
    M_aux = alloc_matrix(n_cities, 4);
...
int64_t ghostPair(int64_t *label_list, int64_t *label_list_inv, int64_t entry) {
    return entry >
                   n_cities - 1
               ? label_list[entry]
...
int64_t tableCode(int64_t ghost_a, int64_t ghost_b, int64_t ghost_c, int64_t a, int64_t b, int64_t c,
                  int64_t common_a, int64_t common_b, int64_t ghost_flag) {
    int64_t ga, gb, gc;
...
void simplifyPaths(int64_t *solution_blue_p2, int64_t n_new, int64_t *vector_comp,
                   int64_t *vector_cand, int64_t *n_entries, int64_t n_cand) {
    int64_t i, j, k, aux, *comp_seq, *inp_comp_seq;
...
void tourTable_fill(int64_t **Tour_table, int64_t *d2_vertices,
                    int64_t *solution_blue_p2, int64_t *solution_red_p2,
                    int64_t *solution_red, int64_t *label_list,
                    int64_t *label_list_inv, int64_t *common_edges_blue_p2,
                    int64_t *common_edges_red_p2, int64_t n_new) {
    int64_t i, sol1, sol2, ghost_a, ghost_b, ghost_c, common_a, common_b,
        common_c, a, b, c;
...
void d2_vertices_id(int64_t *d2_vertices, int64_t *solution_blue_p2,
                    int64_t *common_edges_blue_p2, int64_t n_new) {
    int64_t i;
...
void labelsFix(int64_t *vector_comp, int64_t n_comp, int64_t n_new) {
    int64_t i, j, *gap_labels, *new_label;
...
void tourTable(int64_t *solution_blue_p2, int64_t *solution_red_p2,
    ...
    int64_t i, k, cand_dir, cand_rev, n_comp, sol1, sol2, sol3, sol4, start,
        edge_tour, ghost_pair, n_rounds = 0, n_rounds_max = 1000;
    int64_t min_size_dir, min_size_rev, red_chosen, cand_mcuts, cand_mcuts_dir,
        cand_mcuts_rev, min_size_dir_index, min_size_rev_index;
    int64_t **Tour_table, *assigned_dir, *assigned_rev, *size_dir, *size_rev,
        *vector_comp_red;
    int64_t *d2_vertices, *visited, *recently_assigned, *entries_flag_rev,
    ...
    Tour_table = alloc_matrix(n_new, 6);
    // Tours table
    // lines: vertices;
    // columns:
                        ...
                        if (red_chosen == 0 && assigned_dir[Tour_table[i][4]] != -1) {
                            ...
                            if (vector_comp_red[i] == -1) {
                                ...
                                if (ghost_pair != -1) {
                                    ...
                                    // zero means that it comes from the
                                    // direct red tour
                                    // reversing the ghost nodes (changing
    ...
    dealloc_matrix(Tour_table, n_new);
    free(d2_vertices);
    free(visited);
...
static int64_t *id;    // vector with the id(candidate component) of each node
static int64_t *size;  // vector
static int64_t *n_inputs;
static int64_t *n_outputs;
static tour *blue, *red;
static int64_t n;                              // size of the tours
static int64_t **M_neigh;                      // neighbourhood matrix: first collumn indicates the
                                               // number of neighbours, and the collumns 2 and 3
                                               // indicate the index of the neighbours
static int64_t **M_neigh2;                     // neighbourhood matrix: the collumns indicate the
                                               // number of i conections to the neighbours indicated
                                               // in collumns 2 and 3
int64_t *test;                                 // test of the candidates: 1 - true component; 0 - otherwise
static int64_t isequal(Graph *G1, Graph *G2);  // test if two graphs are equal
...
void new_candidates(int64_t *vector_comp, int64_t n_new) {
    ...
    M_neigh = alloc_matrix(n_cand, 3);
    M_neigh2 = alloc_matrix(n_cand, 2);
    for (i = 0; i < n_cand; i++)
        size[i] = 0;
    for (i = 0; i < n; i++) {
        id[i] = vector_comp[i];
...
void free_candidates(void) {
    ...
    free(n_inputs);
    free(n_outputs);
    dealloc_matrix(M_neigh, n_cand);
    dealloc_matrix(M_neigh2, n_cand);
    for (i = 0; i < n_cand; i++) {
        free(blue[i].inputs);
        free(blue[i].outputs);
        free(red[i].inputs);
...
void findInputs(int64_t *sol_blue, int64_t *sol_red) {
    int64_t i, j, k, aux, aux2, i_l, i_h, comp_size, n_reduc, *sol_blue_reduc,
        *sol_red_reduc, *sol_blue_reduc_t, *sol_red_reduc_t;
    gate_structure gate;

    for (i = 0; i < n_cand; i++) {
        comp_size = (int64_t)ceil(size[i] / 2);  // the inputs/outputs are created
                                                 // with maximum size
...
int64_t isequal(Graph *G1, Graph *G2) {
    int64_t i = 0, G1_empty, G2_empty, equal = 1;
...
void testComp(int64_t cand) {
    int64_t i, *inp_out_blue_inv, *inp_out_red;
...
int64_t testUnfeasibleComp(int64_t *sol_blue) {
    int64_t i, j, aux, *comp_seq, *inp_comp_seq, n_newpart = 0;
...
void fusion(int64_t *sol_blue, int64_t *sol_red) {
    int64_t i, cand, aux, n_fusions = 0, *neigh_vec_cond, *neigh_vec_ind;
...
void fusionB(int64_t *sol_blue, int64_t *sol_red) {
    int64_t i, cand, n_cand_seq = 0, previous_cand, next_cand, n_cand_new,
                     n_rounds = 0, n_rounds_max = 1000;
    int64_t *cand_seq, *cand_seq_cut, *assigned_cand, *new_label,
...
GainType off_gen(int64_t *sol_blue, int64_t *sol_red, int64_t *offspring,
                 int64_t *label_list) {
    int64_t i, i_l, i_h, k, aux, aux2, *select_cand, select_rest,
...
int64_t **alloc_matrix(int64_t lines, int64_t collumns) {
    int64_t i, **Matrix;

    Matrix = new int64_t *[lines]();
    for (i = 0; i < lines; i++)
        Matrix[i] = new int64_t[collumns]();
    if (!Matrix) {
        printf("Allocation Error\n");
        Err(EXIT_FAILURE);
    }
...
const int64_t white = 0, grey = 1, black = 2;

void visitaDfsCC(Graph *g, int64_t u, int64_t *color,
                 int64_t *vector_comp, int64_t components) {
...
void compCon(Graph *g, int64_t *vector_comp) {
    int64_t components = 0, u, n = g->numVertices;
    int64_t *color = new int64_t[n]();
