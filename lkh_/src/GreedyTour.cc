...
static int64_t MayBeAddedToFragments(Node *From, Node *To);
...
static int64_t EdgesInFragments;
...
static Node *NearestNeighbor(Node *From) {
    ...
    int64_t MaxLevel = Dimension, Min = INT_MAX, d;

    if (From->Degree == 2)
        return 0;
    for (auto *NN = From->CandidateSet.data(); (To = NN->To); NN++) {
    ...
    while ((N = First) && N->Level < MaxLevel) {
        ...
        if (N == Last)
            ...
            First = N->OldSuc;
        for (auto *NN = N->CandidateSet.data(); (To = NN->To); NN++) {
            if (To->Mark != mark) {
                To->Mark = mark;
                To->Level = N->Level + 1;
                if (MayBeAddedToFragments(From, To) &&
