...
int64_t Distance_ATT(Node* Na, Node* Nb) {
    ...
    return (int64_t)ceil(Scale * (sqrt((xd * xd + yd * yd) / 10.0)));
}

int64_t Distance_CEIL_2D(Node* Na, Node* Nb) {
    double xd = Na->X - Nb->X, yd = Na->Y - Nb->Y;
    return (int64_t)ceil(Scale * sqrt(xd * xd + yd * yd));
}

int64_t Distance_CEIL_3D(Node* Na, Node* Nb) {
    double xd = Na->X - Nb->X, yd = Na->Y - Nb->Y, zd = Na->Z - Nb->Z;
    return (int64_t)ceil(Scale * sqrt(xd * xd + yd * yd + zd * zd));
}

int64_t Distance_EUC_2D(Node* Na, Node* Nb) {
    double xd = Na->X - Nb->X, yd = Na->Y - Nb->Y;
    return (int64_t)(Scale * sqrt(xd * xd + yd * yd) + 0.5);
}

int64_t Distance_EUC_3D(Node* Na, Node* Nb) {
    double xd = Na->X - Nb->X, yd = Na->Y - Nb->Y, zd = Na->Z - Nb->Z;
    return (int64_t)(Scale * sqrt(xd * xd + yd * yd + zd * zd) + 0.5);
}
...
int64_t Distance_FLOOR_2D(Node* Na, Node* Nb) {
    double xd = Na->X - Nb->X, yd = Na->Y - Nb->Y;
    return (int64_t)floor(Scale * sqrt(xd * xd + yd * yd));
}

int64_t Distance_FLOOR_3D(Node* Na, Node* Nb) {
    double xd = Na->X - Nb->X, yd = Na->Y - Nb->Y, zd = Na->Z - Nb->Z;
    return (int64_t)floor(Scale * sqrt(xd * xd + yd * yd + zd * zd));
}
...
int64_t Distance_GEOM(Node* Na, Node* Nb) {
    ...
    return (int64_t)(Scale * (M_RRR * atan2(sqrt(q1 * q1 + q2 * q2), q5) + 1.0));
}

int64_t Distance_MAN_2D(Node* Na, Node* Nb) {
    return (int64_t)(Scale * (fabs(Na->X - Nb->X) + fabs(Na->Y - Nb->Y)) +
                 0.5);
}

int64_t Distance_MAN_3D(Node* Na, Node* Nb) {
    return (int64_t)(Scale * (fabs(Na->X - Nb->X) +
...
int64_t Distance_GEO_MEEUS(Node* Na, Node* Nb) {
    double lat1 =
        M_PI * ((int64_t)Na->X + 5 * (Na->X - (int64_t)Na->X) / 3) / 180;
    double lon1 =
        M_PI * ((int64_t)Na->Y + 5 * (Na->Y - (int64_t)Na->Y) / 3) / 180;
    double lat2 =
        M_PI * ((int64_t)Nb->X + 5 * (Nb->X - (int64_t)Nb->X) / 3) / 180;
    double lon2 =
        M_PI * ((int64_t)Nb->Y + 5 * (Nb->Y - (int64_t)Nb->Y) / 3) / 180;
    return (int64_t)(Meeus(lat1, lon1, lat2, lon2) + 0.5);
}
...
int64_t Distance_GEOM_MEEUS(Node* Na, Node* Nb) {
    ...
    return (int64_t)(1000 * Meeus(lat1, lon1, lat2, lon2) + 0.5);
}
...
int64_t Distance_TOR_2D(Node* Na, Node* Nb) {
    ...
    return (int64_t)(Scale * sqrt(dx * dx + dy * dy) + 0.5);
}
...
int64_t Distance_TOR_3D(Node* Na, Node* Nb) {
    ...
    return (int64_t)(Scale * sqrt(dx * dx + dy * dy + dz * dz) + 0.5);
}
...
int64_t Distance_XRAY1(Node* Na, Node* Nb) {
    ...
    return (int64_t)(Scale * 100 * cost + 0.5);
}
