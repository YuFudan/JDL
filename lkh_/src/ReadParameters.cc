 ...
 * Default: NO
 *
 * POPMUSIC_MAX_NEIGHBORS = <int64_t>
 * Maximum number of nearest neighbors used as candidates in 3-opt for
 * POPMUSIC.
 * Default: 5
 *
 * POPMUSIC_SAMPLE_SIZE = <int64_t>
 * Sample size.
 * Default: 10
 *
 * POPMUSIC_SOLUTIONS = <int64_t>
 * Number of solutions to be generated.
 * Default: 50
 *
 * POPMUSIC_TRIALS = <int64_t>
...
static char *ReadYesOrNo(int64_t *V);
#undef max
...
void ReadParameters(const char *ParameterFileName) {
    ...
    if (ParameterFileName) {
        if (!(ReadStdin = !strcmp(ParameterFileName, "-")))
    ...
    WriteSolutionToLog = false;
    l_opt_limit = 6;
    waiting_penalty = -1;
    SingleCourier = false;
    while ((Line = ReadStdin ? reader.read_stdin() : ReadLine(ParameterFile))) {
        ...
        if (!strcmp(Keyword, "ALWAYS_WRITE_OUTPUT")) {
            AlwaysWriteOutput = true;
        } else if (!strcmp(Keyword, "WRITE_SOLUTION_TO_LOG")) {
            WriteSolutionToLog = true;
        } else if (!strcmp(Keyword, "L_OPT_LIMIT")) {
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%d", &l_opt_limit))
                eprintf("L_OPT_LIMIT: integer expected");
            if (l_opt_limit < 2 || l_opt_limit > 6)
                eprintf("L_OPT_LIMIT: [2, 6]");
        } else if (!strcmp(Keyword, "WAITING_PENALTY")) {
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%lf", &waiting_penalty))
                eprintf("WAITING_PENALTY: double expected");
            if (waiting_penalty <= 0)
                eprintf("WAITING_PENALTY <= 0");
        } else if (!strcmp(Keyword, "SINGLE_COURIER")) {
            SingleCourier = true;
        } else if (!strcmp(Keyword, "ASCENT_CANDIDATES")) {
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &AscentCandidates))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &BackboneTrials))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &ExternalSalesmen))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &ExtraCandidates))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &InitialStepSize))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &NonsequentialMoveType))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &POPMUSIC_MaxNeighbors))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &POPMUSIC_SampleSize))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &POPMUSIC_Solutions))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &POPMUSIC_Trials))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &MaxPopulationSize))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &SubproblemSize))
            ...
            if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, "%ld", &SubsequentMoveType))
