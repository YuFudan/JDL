...
int64_t SolveSubproblem(int64_t CurrentSubproblem, int64_t Subproblems,
    ...
    int64_t NewDimension = 0, OldDimension = 0, Number, i, InitialTourEdges = 0, AscentCandidatesSaved = AscentCandidates, InitialPeriodSaved = InitialPeriod, MaxTrialsSaved = MaxTrials;
    ...
    if (NewDimension <= 3 || NewDimension == OldDimension) {
        if (TraceLevel >= 1) {
            printff("\nSubproblem %ld of %ld: Dimension = %ld ",
    ...
    do {
        ...
        if (N->FixedTo1 && N->Subproblem != N->FixedTo1->Subproblem)
            eprintf("Illegal fixed edge (%ld,%ld)", N->Id, N->FixedTo1->Id);
        if (N->FixedTo2 && N->Subproblem != N->FixedTo2->Subproblem)
            eprintf("Illegal fixed edge (%ld,%ld)", N->Id, N->FixedTo2->Id);
        N->BestSuc = N->Suc;
    } while ((N = N->Suc) != FirstNode);
    if (TraceLevel >= 1)
        printff("\nSubproblem %ld of %ld: Dimension = %ld, Upper bound = " GainFormat "\n", Number, Subproblems, Dimension, Optimum);
    FreeCandidateSets();
    CreateCandidateSet();
    ...
    for (Run = 1; Run <= Runs; Run++) {
        ...
        if (Cost < Optimum || (Cost != Optimum && OutputTourFileName)) {
            ...
            do {
                if (ProblemType != ATSP)
                    BetterTour[++i] = N->Id;
                else if (N->Id <= Dim_salesman) {
                    i++;
                    if (N->BestSuc->Id != N->Id + Dim_salesman)
                        BetterTour[i] = N->Id;
                    else
                        BetterTour[Dim_salesman - i + 1] = N->Id;
                }
            } while ((N = N->BestSuc) != FirstNode);
            BetterTour[0] =
            ...
            Dimension = NewDimension;
            if (TraceLevel >= 1) {
                printff("*** %ld: Cost = " GainFormat, Number, GlobalCost);
                if (OptimumSaved != -INFINITY && OptimumSaved != 0)
                    printff(", Gap = %0.4f%%",
        ...
        if (TraceLevel >= 1 && Cost != INFINITY)
            printff("Run %ld: Cost = " GainFormat ", Time = %0.6f sec.\n\n",
                    Run, Cost, Time);
        if (PopulationSize >= 2 &&
