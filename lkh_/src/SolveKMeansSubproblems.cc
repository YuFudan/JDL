...
static void KMeansClustering(int64_t K);
...
static void KMeansClustering(int64_t K) {
    ...
    int64_t *Count, i, j, d, OldSubproblem;
    double *SumXc, *SumYc, *SumZc, Xc, Yc, Zc;
    int64_t *Movement, *MMax, Max;
    int64_t Moving = 0;
    CostFunction OldDistance = Distance;

    Center = new Node[K + 1]();
    CenterRef = new Node *[K + 1]();
    SumXc = new double[K + 1]();
    SumYc = new double[K + 1]();
    SumZc = new double[K + 1]();
    Count = new int64_t[K + 1]();
    Movement = new int64_t[K + 1]();
    MMax = new int64_t[K + 1]();
    Perm = new Node *[Dimension]();
    ...
    for (;;) {
        ...
        do {
            ...
            if (N->M < 0) {
                ...
                if (OldSubproblem == 0)
                    ...
                    N->Cost =
                        Distance(N, &Center[N->Subproblem]);
                    N->NextCost =
                        Distance(N, &Center[N->LastV]);
                    ...
                    if ((!c || c(N, &Center[i]) <= N->Cost) &&
                        ...
                        if (d < N->NextCost)
                            N->Subproblem = i;
                               ...
                               (d != INT_MIN || (d = Distance(N, &Center[i])) < N->NextCost)) {
                        N->NextCost = d;
                        N->LastV = i;
                    }
                }
        ...
        for (i = 1; i <= K; i++) {
            if (Count[i] > 0) {
                ...
                Movement[i] = Distance(&Old, &Center[i]);
                if (Movement[i] > Max)
                    Max = Movement[i];
            } else
                Movement[i] = 0;
