...
static void NearestQuadrantNeighbors(Node *N, int64_t Q, int64_t K);
static int64_t Contains2D(Node *T, int64_t Q, Node *N);
static int64_t Contains3D(Node *T, int64_t Q, Node *N);
static int64_t BoxOverlaps2D(Node *T, int64_t Q, Node *N);
static int64_t BoxOverlaps3D(Node *T, int64_t Q, Node *N);
static void ComputeBounds(int64_t start, int64_t end);

typedef int64_t (*ContainsFunction)(Node *T, int64_t Q, Node *N);
typedef int64_t (*BoxOverlapsFunction)(Node *T, int64_t Q, Node *N);

static Node **KDTree;
static std::vector<Candidate> CandidateSet;
static double *XMin, *XMax, *YMin, *YMax, *ZMin, *ZMax;
static int64_t Candidates, Radius;
...
void CreateQuadrantCandidateSet(int64_t K) {
    ...
    XMin = new double[1 + Dim_salesman]();
    XMax = new double[1 + Dim_salesman]();
    YMin = new double[1 + Dim_salesman]();
    YMax = new double[1 + Dim_salesman]();
    if (CoordType == THREED_COORDS) {
        ZMin = new double[1 + Dim_salesman]();
        ZMax = new double[1 + Dim_salesman]();
    }
    ...
    if (Level == 0 &&
        ...
        std::vector<std::vector<Candidate>> SavedCandidateSet(1 + Dim_salesman);
        if (TraceLevel >= 2)
            printff("done\n");
        From = FirstNode;
        while ((From = From->Suc) != FirstNode)
        ...
        if (From != FirstNode) {
            ...
            From = FirstNode;
            do {
                SavedCandidateSet[From->Id].swap(From->CandidateSet);
                From->CandidateSet.clear();
                From->Zc = From->Y;
                if (WeightType == GEO || WeightType == GEO_MEEUS)
                    From->Y =
                        (int64_t)From->Y + 5.0 * (From->Y - (int64_t)From->Y) / 3.0;
                From->Y += From->Y > 0 ? -180 : 180;
                if (WeightType == GEO || WeightType == GEO_MEEUS)
                    From->Y =
                        (int64_t)From->Y + 3.0 * (From->Y - (int64_t)From->Y) / 5.0;
            } while ((From = From->Suc) != FirstNode);
            Level++;
            ...
            do {
                SavedCandidateSet[From->Id].swap(From->CandidateSet);
                auto &QCandidateSet = SavedCandidateSet[From->Id];
                for (auto &NFrom : QCandidateSet) {
                    if (!NFrom.To) break;
                    AddCandidate(From, NFrom.To, NFrom.Cost, NFrom.Alpha);
                }
            } while ((From = From->Suc) != FirstNode);
        }
    }
...
static void ComputeBounds(int64_t start, int64_t end) {
    if (start <= end) {
        int64_t mid = (start + end) / 2, i;
        Node *T = KDTree[mid];
        XMin[T->Id] = YMin[T->Id] = INFINITY;
        XMax[T->Id] = YMax[T->Id] = -INFINITY;
        if (CoordType == THREED_COORDS) {
            ZMin[T->Id] = INFINITY;
            ZMax[T->Id] = -INFINITY;
        }
...
static int64_t Overlaps(int64_t Q, double diff, int64_t High, int64_t axis) {
    switch (Q) {
        case 1:
            return High || diff >= 0;
        case 2:
...
static int64_t InCandidateSet(Node *N, Node *T) {
    int64_t i;
    for (i = 0; i < Candidates; i++)
...
static void NQN(Node *N, int64_t Q, int64_t start, int64_t end, int64_t K) {
    int64_t mid = (start + end) / 2, d;
    ...
    if (start <= end && T != N && !T->FixedTo2 &&
        ...
        (d = Distance(N, T)) <= Radius) {
        int64_t i = Candidates;
