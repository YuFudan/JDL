...
static int64_t SierpinskiIndex(double x, double y);
static int64_t MooreIndex(double x, double y);
static int compare(const void *Na, const void *Nb);

typedef int64_t (*IndexFunction)(double x, double y);
...
GainType SFCTour(int64_t CurveType) {
    ...
    do {
        N->LastV = 1;
        if (!FixedOrCommon(N, N->Suc) && !N->CandidateSet.empty()) {
...
static int64_t MooreIndex(double x, double y) {
    static const int64_t Rank[5][4] =
        {{1, 0, 2, 3}, {2, 3, 1, 0}, {2, 1, 3, 0}, {0, 3, 1, 2}, {0, 1, 3, 2}};
    static const int64_t NextState[5][4] =
        {{2, 2, 3, 3}, {1, 3, 1, 2}, {2, 2, 4, 1}, {4, 1, 3, 3}, {3, 4, 2, 4}};
    int64_t idx = 0, State = 0;
    int64_t i = 4 * sizeof(int64_t);
...
static int compare(const void *Na, const void *Nb) {
    int64_t NaV = (*(Node **)Na)->V;
    int64_t NbV = (*(Node **)Nb)->V;
