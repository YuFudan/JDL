...
static GainType PatchCyclesRec(int64_t k, int64_t m, int64_t M, GainType G0);
static int64_t ShortestCycle(int64_t M, int64_t k);
static int64_t Cycle(Node *N, int64_t k);

static int64_t CurrentCycle, Patchwork = 0, RecLevel = 0;
...
static GainType PatchCyclesRec(int64_t k, int64_t m, int64_t M, GainType G0) {
    ...
    GainType G1, G2, G3, G4, Gain, CloseUpGain,
        BestCloseUpGain = PatchingAExtended ? -INFINITY : 0;
    int64_t X4, X6;
    int64_t i, NewCycle, *cycleSaved = 0, *pSaved = 0;
    int64_t Breadth2 = 0, Breadth4;
    ...
    for (auto *Ns2 = s2->CandidateSet.data(); (s3 = Ns2->To); Ns2++) {
        ...
        for (X4 = 1; X4 <= 2; X4++) {
            ...
            if (M > 2) {
                if (!cycleSaved) {
                    cycleSaved = new int64_t[2 * k]();
                    memcpy(cycleSaved, cycle + 1, 2 * k * sizeof(int64_t));
                }
                for (i = 1; i <= 2 * k; i++)
                    if (cycle[i] == NewCycle)
                ...
                memcpy(cycle + 1, cycleSaved, 2 * k * sizeof(int64_t));
                ...
                if (CurrentPenalty != 0 || TSPTW_Makespan || Gain > 0) {
                    if (!pSaved) {
                        pSaved = new int64_t[2 * k]();
                        memcpy(pSaved, p + 1, 2 * k * sizeof(int64_t));
                    }
                    ...
                    memcpy(p + 1, pSaved, 2 * k * sizeof(int64_t));
                    for (i = 1; i <= 2 * k; i++)
                        q[p[i]] = i;
                }
            }
    ...
    if (M == 2 && !PatchingCRestricted) {
        ...
        for (auto *Ns2 = s2->CandidateSet.data(); (s3 = Ns2->To); Ns2++) {
            ...
            for (X4 = 1; X4 <= 2; X4++) {
                ...
                for (auto *Ns4 = s4->CandidateSet.data(); (s5 = Ns4->To); Ns4++) {
...
static int64_t ShortestCycle(int64_t M, int64_t k) {
    int64_t i, Cycle, MinCycle = 0;
    int64_t *Size, MinSize = INT_MAX;

    Size = new int64_t[1 + M]();
    p[0] = p[2 * k];
    for (i = 0; i < 2 * k; i += 2)
