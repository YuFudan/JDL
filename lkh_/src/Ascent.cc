...
GainType Ascent() {
    ...
    int64_t T, Period, P, InitialPhase, BestNorm;
...
Start:
    ...
    if (CandidateSetType == DELAUNAY && FirstNode->CandidateSet.empty())
        CreateDelaunayCandidateSet();
    else if (CandidateSetType == POPMUSIC && FirstNode->CandidateSet.empty())
    ...
    if (MaxCandidates > 0) {
        ...
        MaxAlpha = INT_MAX;
        if (Optimum != -INFINITY && (Alpha = Optimum - W) >= 0)
            MaxAlpha = Alpha;
    ...
    for (Period = InitialPeriod, T = InitialStepSize;
         Period > 0 && T > 0 && Norm != 0; Period /= 2, T /= 2) {
        /* Period and step size are halved at each iteration */
        if (TraceLevel >= 2)
            printff("  T = %ld, Period = %ld, BestW = %ld, BestNorm = %ld\n",
                    T, Period, BestW, BestNorm);
        for (P = 1; T && P <= Period && Norm != 0; P++) {
            ...
            if (W > BestW || (W == BestW && Norm < BestNorm)) {
                ...
                if (TraceLevel >= 3)
                    printff("  T = %ld, Period = %ld, P = %ld, W = %ld, Norm = %ld\n",
                            T, Period, P, W, Norm);
    ...
    if (MaxCandidates > 0 && CandidateSetType != POPMUSIC) {
        ...
        t = FirstNode;
        do {
            for (Nt = t->CandidateSet.data(); Nt && Nt->To; Nt++)
                Nt->Cost += t->Pi + Nt->To->Pi;
        } while ((t = t->Suc) != FirstNode);
    }
    if (TraceLevel >= 2)
        printff("Ascent: BestW = %0.1f, Norm = %ld\n",
                (double)BestW, Norm);
    return W;
}
