 ...
 *      int64_t Cost    : the cost (length) of the edge
 *      int64_t Alpha   : the alpha-value of the edge
 *
...
void GenerateCandidates(int64_t MaxCandidates, GainType MaxAlpha,
                        int64_t Symmetric) {
    Node *From, *To;
    int64_t a, d, Count;
    ...
    if (MaxCandidates > 0) {
        do {
            ...
            From->CandidateSet.resize(MaxCandidates + 1);
        } while ((From = From->Suc) != FirstNode);
    } else {
        AddTourCandidates();
        do {
    ...
    do {
        auto *NFrom = From->CandidateSet.data();
        if (From != FirstNode) {
            From->Beta = INT_MIN;
