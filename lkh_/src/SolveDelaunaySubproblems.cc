...
static int64_t DelaunayClustering(int64_t MaxClusterSize);
...
static int compareCost(const void *e1, const void *e2) {
    int64_t d1 = ((Edge *)e1)->Cost;
    int64_t d2 = ((Edge *)e2)->Cost;
    return d1 < d2 ? -1 : d1 == d2 ? 0 : 1;
}

static int compareFromTo(const void *e1, const void *e2) {
    int64_t From1 = ((Edge *)e1)->From->Id;
    int64_t To1 = ((Edge *)e1)->To->Id;
    int64_t From2 = ((Edge *)e2)->From->Id;
    int64_t To2 = ((Edge *)e2)->To->Id;
...
static int64_t DelaunayClustering(int64_t MaxClusterSize) {
    int64_t Count = 0, Count1, Count2 = 0, i, j = 0;
    Edge *EdgeSet, Key;
    Node *From, *To, *N, *F, *T;
    point *u, *v;
    edge *e_start, *e;
    ...
    for (i = 0; i < Dimension; i++) {
        ...
        do {
            ...
            if (u->id < v->id) {
                ...
                EdgeSet[j++].Cost = FixedOrCommon(From, To) ? INT_MIN : Distance(From, To) + From->Pi + To->Pi;
            }
        } while ((e = Next(e, u)) != e_start);
    }
    free_memory();
            ...
            for (i = 0; i < Dimension; i++) {
                ...
                do {
                    ...
                    if (!bsearch(&Key, EdgeSet, Count1, sizeof(Edge),
                        ...
                        EdgeSet[Count].To = To;
                        EdgeSet[Count].Cost =
                            FixedOrCommon(From, To) ? INT_MIN : Distance(From, To) + From->Pi + To->Pi;
                        Count++;
                    }
                } while ((e = Next(e, u)) != e_start);
            }
