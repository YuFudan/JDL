...
static void Distribute(int64_t Constrained, double R);
...
static Saving *S;
static int64_t Sets, SSize;

GainType CVRP_InitialTour() {
    Node *N, *Last, *Next, *Tour;
    int64_t s, Dim = Dimension - Salesmen + 1, it;
    GainType Cost, BestCost = INFINITY, BestPenalty = INFINITY;
...
void CreateS() {
    int64_t Dim = Dimension - Salesmen + 1, i, j;
    Node *Ni, *Nj;
    SSize = 0;
    S = new Saving[(Dim - 2) * (Dim - 1) / 2]();
    /* Compute savings */
    for (i = 1; i < Dim; i++) {
        Ni = &NodeSet[i];
        if (Ni == Depot)
...
static void Union(Node *x, Node *y) {
    ...
    int64_t d = x == y || x->DepotId || y->DepotId ? 0 : OldDistance(x, y) - OldDistance(x, Depot) - OldDistance(y, Depot);
    if (u->Size < v->Size) {
        u->Dad = v;
        v->Size += u->Size;
        v->Load += u->Load;
...
static int compareValue(const void *s1, const void *s2) {
    int64_t v1 = ((Saving *)s1)->Value;
    int64_t v2 = ((Saving *)s2)->Value;
