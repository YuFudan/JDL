...
def plot_buildings(buildings, regions=None, site=None, path="figure/test.html"):
...
def read_site():
    """漫香林所管辖的路区总片区围栏"""
    ps = json.load(open("orig_data/siteFence_geojson.json"))["features"][0]["geometry"]["coordinates"][0]
    site = {
        "name": "漫香林营业部",
...
def read_poi_community_data(poi_data, community_data):
    ...
    G_folium.save("figure/poi_community.html")
...
def get_building_semantics(buildings, pois, communities):
    ...
    for b in tqdm(buildings):
        if b["name"] is None:
            ...
            if b["floor"] is None:
                not_known_floor_cnt += 1  
    print("building not_known_name_cnt:", not_known_name_cnt, "/", query_cnt)
...
if __name__ == "__main__":
    ...
    # communities = pickle.load(open("data/poi_community.pkl", "rb"))[1]
    # cnt = 0
    # for b in buildings:
    #     b["comm_id"] = find_bd_in_which_comm(b, communities)
    #     if b["comm_id"] is None:
    #         cnt += 1
    # print("not in any comm:", cnt, "/", len(buildings))
    ...
    #     } for bd in buildings
    # ]
    # json.dump(node_geojson, open("data/nodes_geojson.json", "w"))
    # exit()

    # 读有留白aoi
    orig_data = json.load(open("orig_data/building_geojson.json"))["features"]
    buildings = read_building_data(orig_data)
    print("buildings:", len(buildings))

    # 读无留白aoi
    orig_data_nogap = json.load(open("orig_data/noGapBuilding_geojson.json"))["features"]
    ...
    #     regions=regions, 
    #     site=site, 
    #     path="figure/buildings_nogap.html"
    # )

    # 将无留白的信息直接加到有留白的里面
    id_mapping = open("orig_data/manxianglin_idmapping.csv").readlines()
    ...
    path = "data/poi_community.pkl"
    if os.path.exists(path):
        pois, communities = pickle.load(open(path, "rb"))
    else:
        poi_data = json.load(open("orig_data/POI_geojson.json"))["features"]
        community_data = json.load(open("orig_data/xiaoqu_geojson.json"))["features"]
    ...
    pickle.dump(buildings_final, open("data/buildings.pkl", "wb"))
    json.dump(buildings_final, open("data/buildings.json", "w"))

    buildings_final = pickle.load(open("data/buildings.pkl", "rb"))
    plot_buildings(
        buildings=buildings_final,
        regions=regions, 
        site=site, 
        path="figure/buildings_final.html"
    )
